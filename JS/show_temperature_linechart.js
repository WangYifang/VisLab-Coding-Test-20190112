show_temperature_linechart = function (rawdata, id, tag, max_temperature_array, min_temperature_array){
    var flag;
    if(tag == "max"){
    ...
    //deal with data
    //processedData: [year, month, max_tempature_average, min_tempature_average, max_temperature_array, min_temperature_array]
    var processedData = [];
    ...
    var count = 0;
    max_temperature_array.forEach(element => {
        processedData[count][4] = element;
        count++;
    });
    var count = 0;
    min_temperature_array.forEach(element => {
      processedData[count][5] = element;
      count++;
    });
    ...
    // x axis
    var x = d3.scaleBand().range([0, width/1.2]);
    ...
    // Line Chart
    var xScale_local = d3.scaleTime().range([0, gridSize*2]);
    var yScale_local = d3.scaleLinear().range([gridSize, 0]);
    var formatTime2Date = d3.timeFormat("%d");
    xScale_local.domain(["01", "31"]);
    yScale_local.domain([0, 40]);

    // Accessor Function
    var lineFunction = d3.line()
        .x(function (d) { var date = new Date(d.x); return xScale_local(formatTime2Date(date)); })      
        .y(function (d) { return yScale_local(parseFloat(d.y)); });
    
    var temp = svg.append("g").selectAll("path")
        .data(processedData)
        .enter()
        .append('g');    

    // max temperature
    var maxPath = temp.append("path")
      .attr("d", function(d, i) {
        return lineFunction(processedData[i][4]);
      })
      .attr("stroke", "#5698c3")
      .attr("stroke-width", 1)
      .attr("fill", "none")
      .attr("transform", function(d) {
        return `translate(${margin * 10 + gridSize / 3.5 + xScale(d[0])},${margin * 5 + gridSize / 3.5 + yScale(d[1])})`;
      });
    
    // min temperature
    var minPath = temp.append("path")
        .attr("d", function (d, i) {
            return lineFunction(processedData[i][5]);
        })
        .attr("stroke", "#add5a2")
        .attr("stroke-width", 1)
        .attr("fill", "none")
    ...
    var legend = svg.append("g")
        ...
        .attr("transform", `translate(${margin * 12 + gridSize / 3 + width / 1.2}, ${margin * 7})`);
    legend.append("text")
